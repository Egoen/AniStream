...
import { useCompleted } from "@/hooks/useAnimeQuery";
  ...
  return (
    <Link to={`/anime/${anime.slug}`} className="group cursor-pointer">
      <div className="relative aspect-[3/4] rounded-xl overflow-hidden mb-4 transition-all duration-300 group-hover:scale-[1.03] group-hover:shadow-[0_0_20px_rgba(255,140,148,0.2)]">
        ...
        {/* Completed badge */}
        <div className="absolute top-3 left-3 bg-emerald-600 text-[10px] font-black px-2 py-1 rounded text-white tracking-widest">
          COMPLETED
        </div>
        ...
        {anime.totalEpisode && (
          <div className="absolute bottom-3 left-3">
            <span className="text-[10px] font-bold text-emerald-300 bg-emerald-950/80 px-2 py-0.5 rounded backdrop-blur-md">
              {anime.totalEpisode} EPS
            </span>
          </div>
        )}
        ...
        {anime.rating && !isNaN(Number(anime.rating)) && (
          <div className="absolute top-3 right-3">
            <span className="text-[10px] font-black text-amber-300 bg-black/70 px-2 py-0.5 rounded backdrop-blur-md flex items-center gap-0.5">
      ...
      <p className="text-xs text-[#a9abaf] mt-1 line-clamp-1">
        {anime.genres?.slice(0, 2).map((g) => g.name).join(" • ") || anime.latestReleaseDate || ""}
      </p>
    </Link>
  );
}
...
export default function CompletedPage() {
  ...
  const { data, isLoading, isError } = useCompleted(page);
    ...
    <div className="bg-[#0b0e11] min-h-screen">
      <main className="max-w-7xl mx-auto px-6 md:px-8 pt-28 pb-12">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div>
            <span className="text-[#ff8c94] font-bold tracking-widest text-xs mb-2 block font-headline">ARCHIVE</span>
            <h1 className="text-5xl font-extrabold tracking-tight font-headline text-[#f8f9fe]">Completed Anime</h1>
            <p className="text-[#a9abaf] text-sm mt-2">Full series — binge-watch from start to finish.</p>
          </div>
          <div className="flex flex-wrap gap-3 items-center bg-[#101417] p-2 rounded-xl">
            <FilterDropdown label="Genre" value="All" />
            <FilterDropdown label="Sort" value="Latest" />
        ...
        {!isLoading && !isError && (hasMore || page === 1) && displayed.length > 0 && (
          <div className="flex justify-center mt-16">
            ...
            >
              {loadingMore ? (
                <><Loader2 size={16} className="animate-spin text-black" />Loading...</>
              ) : "Load More"}
            </button>
          </div>
        )}
        ...
        <footer className="border-t border-[#22262b] mt-24 pt-16 pb-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-12">
            ...
            <div>
              ...
              <ul className="space-y-3">
                {[["Home", "/"], ["Ongoing", "/ongoing"], ["Genre", "/genre"], ["Schedule", "/schedule"]].map(([label, href]) => (
          ...
          <div className="flex items-center justify-between pt-6 border-t border-[#22262b]">
            ...
            <div className="flex items-center gap-4">
              ...
              <Link to="/history" className="text-[#737679] hover:text-[#ff8c94] transition-colors text-sm">History</Link>
            </div>
          </div>
        </footer>
      </main>
